...
| mock | はい | boolean。文字列の "true" / "false" は不可 |
...
開発ビルドはmock: trueを許可し、CLIで対象を警告する。ページにはモック注意書きを表示しない。公開用のbuild:releaseでは1件でもtrueなら失敗させる。
...
3. 事実確認と仮文言の削除を完了してからmockをfalseにする。falseへの一括置換だけで完了扱いにしない。
